...
insert into memurlar1 values(1,'Ali','Ozturk',50000,100);
insert into memurlar1 values(2,'Adem','Tan',40000,90);
insert into memurlar1 values(3,'Halit','Ozyıldızlı',35000,85);
insert into memurlar1 values(4,'Hasan','Oz',45000,95);
insert into memurlar1 values(5,'Recep','Cicek',30000,80);
insert into memurlar1 values(6,'Harun','Sever',20000,70);
insert into memurlar1 values(7,'Hatice','Tarkan',25000,75);
insert into memurlar1 values(8,'Poyraz','Yuzbası',10000,60);
insert into memurlar1 values(9,'Ayse','Karlı',15000,65);
insert into memurlar1 values(10,'Adem','Akmaz',9000,55);
-- SORU1: memurlar tablosunda id'si 5'den buyuk olan,
-- ilk 2 veriyi listeleyiniz
select * from memurlar1 where id>5 limit 2;
--2. YOL
select * from memurlar1 where id>5 fetch next 2 row only;
-- SORU2: memurlar tablosunda sicil_notu'i en yuksek 3 kisinin bilgilerini listeleyiniz
select * from memurlar1 order by sicil_notu desc limit 3;
select * from memurlar1 order by sicil_notu desc fetch next 3 row only;
-- SORU3: memurlar tablosunda maas'ı en dusuk 2 kisinin bilgilerini listeleyiniz
select * from memurlar1 order by maas limit 2;
select * from memurlar1 order by maas fetch next 2 row only;
-- SORU4: memurlar tablosunda sicil_notu en yuksek 3.4.5. kişilerin bilgilerini listeleyiniz
select * from memurlar1 order by sicil_notu desc offset 2 limit 3;
select * from memurlar1 order by sicil_notu desc offset 2 fetch next 3 row only;
-- SORU5: memurlar tablosunda soyisim'i en uzun ilk 2 kisinin bilgilerini listeleyiniz
select * from memurlar1 order by length(soyisim) desc limit 2;
select * from memurlar1 order by length(soyisim) desc fetch next 2 row only;
...
INSERT INTO calisanlar2 VALUES('Ali Seker', 'Istanbul', 2500, 'Vakko');
INSERT INTO calisanlar2 VALUES('Ayse Gul', 'Istanbul', 1500, 'LCWaikiki');
INSERT INTO calisanlar2 VALUES('Veli Yilmaz', 'Ankara', 3000, 'Vakko');
INSERT INTO calisanlar2 VALUES('Veli Yilmaz', 'Ankara', 7000, 'Adidas');
INSERT INTO calisanlar2 VALUES('Nisa Can', 'Ankara', 1500, 'Pierre Cardin');
INSERT INTO calisanlar2 VALUES('Fatma Yasa', 'Bursa', 2500, 'Vakko');
...
select isyeri, isim, maas, sehir from calisanlar2
...
select marka_isim, calisan_sayisi,(select sum(maas) as toplam_maas from calisanlar2 where marka_isim=isyeri) 
from markalar;
...
(select min(maas) as min_maas from calisanlar2 where marka_isim=isyeri) 
from markalar;
